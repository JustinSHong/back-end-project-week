...
router.route("/").get((req, res) => {
    ...
    query
        ...
        .catch(err => {
            res.status(500).json({
                errorMessage: "Notes could not be retrieved"
            });
        });
});
...
router.route("/").post((req, res) => {
    if (!req.body.title || !req.body.content) {
        res.status(400).json({
            errorMessage: "Please provide a title and text for the note"
        });
    } else {
        ...
        newNote
            ...
            .catch(err => {
                res.status(500).json({
                    errorMessage:
                        "There was an error saving the note to the database"
                });
            });
    }
});
...
router.route("/:id").delete((req, res) => {
    ...
    const query = Note.findByIdAndRemove(id)
        ...
        .catch(err => {
            res.status(500).json({
                errorMessage: "The note could not be removed"
            });
        });
});
...
router.route("/:id").put((req, res) => {
    ...
    if (!update.title || !update.content) {
        ...
        query
            ...
            .catch(err => {
                res.status(500).json({
                    errorMessage: "The note data could not be modified"
                });
            });
    }
});
